...
double variable_t::Adaptacion(double x) const {
  ...
  double valor = (*dominio).Adaptacion(x);
  if (valor == 0) {
    cout << "No tiene adaptacion con ninguna etiqueta\n";
  }
  return valor;
}
...
double variable_t::AdaptacionSinNormalizar(double x, string etiquetas) const {
  if (dominio==0){
    cout << "Variable sin dominio asociado\n";
    exit(1);
  }
...
// Operador que devuelve el grado de adaptaci�n NORMALIZADA de un determinado valor x
// a un grupo de etiquetas "etiquetas" de la variable. El par�metro etiquetas
// viene expresado como un vector ordenado (cadena caracteres string) de ceros
// y unos, representando ausencia y presencia respectivamente.
double variable_t::AdaptacionNorm(double x, string etiquetas) const {
  if (dominio==0){
    cout << "Variable sin dominio asociado\n";
    exit(1);
  }
  return (*dominio).AdaptacionNorm(x,etiquetas);
}
