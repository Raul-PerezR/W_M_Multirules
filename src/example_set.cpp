...
void example_set::OrderByClass(int order, int VarClase)
{ // order=0 -> as it is, order=1 -> by class in order
  int old_n_clase = 0;
  ...
  break;
  case 2:
  { // Ordered by class (one at a time... 1 2 3)
...
{
  vector<double> n_example_of;
  for (int i = 0; i < nclasses; i++)
    n_example_of.push_back(0);
...
example_set example_set::Extract_Training_Set(int particion, int VarClase, int order) const
{
  ...
  {
  case 1:
    E1.OrderByClass(1, VarClase);
    break;
  case 2:
    E1.OrderByClass(2, VarClase);
    break;
  default:
    break;
  }
...
example_set example_set::Extract_Training_Set2(int particion1, int particion2, int VarClase, int order) const
{
  int sum = 0;
  for (int i = 0; i < n_example; i++)
    if (partition[i] != particion1 and partition[i] != particion2)
      sum++;
...
example_set example_set::Extract_Test_Set2(int particion1, int particion2, int VarClase) const
{
  int sum = 0;
  for (int i = 0; i < n_example; i++)
    if (partition[i] == particion1 or partition[i] == particion2)
      sum++;
