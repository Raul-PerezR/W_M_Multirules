...
void domain_t::Asigna(int n, double inf, double sup, double *a, double *b, double *c, double *d, string *name){
  numero=n;
  rango_inf=inf;
  rango_sup=sup;
  inf_inf=true;
...
// [a[i],b[i],c[i],d[i]] representa la etiqueta i-esima del dominio.
// el vector name contiene el nombre de cada etiqueta.
void domain_t::Asigna_Closed_Interval(int n, double inf, double sup, double *a, double *b, double *c, double *d, string *name){
  numero=n;
  rango_inf=inf;
  rango_sup=sup;
  inf_inf=true;
...
double domain_t::Adaptacion(double x, string etiquetas) const{
  ...
  for (int i=0; i<numero and mayor<1; i++)
    if (etiquetas[i]=='1'){
        double aux = label[i].Adaptacion(x);
        //label[i].Pinta();
        //cout << " {x= " << x << " aux= " << aux <<"} ";
        if (aux > mayor){
          mayor = aux;
...
// Operaci�n que evalua el grado de adaptaci�n normalizado de un determinado valor x
// a un conjunto de etiquetas del dominio. Las etiquetas se expresan como
// un vector ordenado (cadena de caracteres <string>) de ceros y unos. El cero
// representa ausencia y el uno presencia.
double domain_t::AdaptacionNorm(double x, string etiquetas) const{
  int netiq;
  if (numero==0) {
    cout << "El dominio no esta creado\n";
    exit(1);
  ...
  netiq=etiquetas.length();
  if (netiq>numero){
    cout << "secuencia de etiquetas no coincide con el tamano del dominio\n";
    exit(1);
  }

  double valorMax=0, valor=0, aux;
  for (int i=0; i<numero and valorMax<1; i++){
    aux = label[i].Adaptacion(x);
    if (etiquetas[i]=='1' and aux > valor) valor = aux;
    if (aux > valorMax) valorMax = aux;
  }
...
string domain_t::SPinta(int i) const {
  return label[i].SPinta();
}
