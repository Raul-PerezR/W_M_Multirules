...
genetcode::genetcode (int NPobBin, int NPobEnt, int NPobRea, int *tama){
  binary=NPobBin;
...
string genetcode::GetKey(){
  char *s = new char[TamaBinary(0)+1];
  for (int i=0; i<TamaBinary(0); i++){
    s[i]=GetValueBinary(1,i);
  }
  s[TamaBinary(0)]='\'0';
